...
public class LoggingInterceptor implements HandlerInterceptor {
    ...
    private final String pod_id = UUID.randomUUID().toString();
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        ThreadContext.put("pod_version", "1.0.0"); //TODO: get version from JAR MANIFEST
        ...
        try {
            ThreadContext.put("request_body", request.getReader().lines().collect(Collectors.joining(System.lineSeparator())));
        } catch (Exception e) {
    ...
    private String maskAuthToken(HttpServletRequest request) {
        String authToken = Optional.ofNullable(request.getHeader("X-Auth-Token")).orElse("00000000-0000-0000-0000-000000000000");
        int length = authToken.length();
        if (length <= 6) {
            return authToken;
        }
        return authToken.substring(0, 3) + "****" + authToken.substring(length - 3);
    }
}